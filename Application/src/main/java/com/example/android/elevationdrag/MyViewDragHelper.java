...
public class MyViewDragHelper {
    ...
    /**
     * A Callback is used as a communication channel with the MyViewDragHelper back to the
     * parent view using it. <code>on*</code>methods are invoked on siginficant events and several
     * accessor methods are expected to provide the MyViewDragHelper with more information
    ...
    public static abstract class Callback {
         ...
         * one of these methods, the MyViewDragHelper will enter {@link #STATE_SETTLING}
         * and the view capture will not fully end until it comes to a complete stop.
         * If neither of these methods is invoked before <code>onViewReleased</code> returns,
         * the view will stop in place and the MyViewDragHelper will return to
         * {@link #STATE_IDLE}.</p>
         *
         ...
         *
         * <p>MyViewDragHelper may call this method multiple times for the same view even if
    ...
    /**
     * Factory method to create a new MyViewDragHelper.
     *
     * @param forParent Parent view to monitor
     * @param cb Callback to provide information and receive events
     * @return a new MyViewDragHelper instance
     */
    ...
    public void captureChildView(View childView, int activePointerId) {
        if (childView.getParent() != mParentView) {
            throw new IllegalArgumentException("captureChildView: parameter must be a descendant " +
                    "of the MyViewDragHelper's tracked parent view (" + mParentView + ")");
        }
     ...
     * of the MyViewDragHelper's knowledge).
     *
    ...
    public boolean shouldInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEventCompat.ACTION_POINTER_DOWN: {
                ...
                // A MyViewDragHelper can only manipulate one view at a time.
                if (mDragState == STATE_IDLE) {
                    final int edgesTouched = mInitialEdgesTouched[pointerId];
                    if ((edgesTouched & mTrackingEdges) != 0) {
                        mCallback.onEdgeTouched(edgesTouched & mTrackingEdges, pointerId);
    ...
    public View findTopChildUnder(int x, int y) {
        ...
        final int tolerance = Math.round(convertDpToPixel(10));
        ...
        for (int i = childCount - 1; i >= 0; i--) {
            ...
            if (x >= child.getLeft()-tolerance && x < child.getRight()+tolerance &&
                ...
                Log.i("left ", child.getLeft()+"");
                String tag = (String)child.getTag();
                if (tag != null) {
                    if (tag.contains("touchThrough"))
                        continue;
                }
                return child;
            }
